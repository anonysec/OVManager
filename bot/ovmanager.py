...
class OVManager:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if attempt == MAX_RETRIES - 1:
                    ...
                    raise
                logger.warning("GET %s retry %d: %s", path, attempt + 1, e)
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if attempt == MAX_RETRIES - 1:
                    ...
                    raise
                logger.warning("POST %s retry %d: %s", path, attempt + 1, e)
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if attempt == MAX_RETRIES - 1:
                    ...
                    raise
                logger.warning("DELETE %s retry %d: %s", path, attempt + 1, e)
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if attempt == MAX_RETRIES - 1:
                    ...
                    raise
                logger.warning("PATCH %s retry %d: %s", path, attempt + 1, e)

    # ── DB access (runs in-panel, no HTTP needed) ──
    ...
    def _get_status_from_db(self):
        try:
            ...
            try:
                info = {}
                r = db.execute(text("SELECT COUNT(*) as total, SUM(used) as used FROM users")).fetchone()
                if r:
                    info["users"] = {"total": r.total or 0, "used": r.used or 0}
                r = db.execute(text("SELECT COUNT(*) as cnt FROM nodes")).fetchone()
    ...
    def _get_users_from_db(self) -> list:
        try:
            ...
            try:
                return [
                    {k: getattr(u, k) for k in u.__table__.columns.keys()}
    ...
    async def get_user(self, username: str):
        for u in self._get_users_from_db():
    ...
    def _get_nodes_from_db(self):
        try:
            ...
            try:
                return [
                    {k: getattr(n, k) for k in n.__table__.columns.keys()}
    ...
    async def get_node(self, node_id: int):
        for n in self._get_nodes_from_db():
            if n.get("id") == node_id:
                return n
        return {}

    async def download_config(self, username: str, node_name: str):
        """Download .ovpn config for a user. Returns file content or None."""
        try:
            from backend.db.engine import SessionLocal
            from backend.db import models
